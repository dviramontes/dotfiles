...
- `--repo ` -- match the live session by its current loaded repo root (most common)
- ` ` -- match by exact ID (use when multiple sessions share a repo)
- If only one session exists, it auto-resolves

`reload` also supports:

- `--session-path ` -- match the live Hunk window by its current working directory
- `--source ` -- load the replacement `diff` / `show` command from a different directory
...
- `--repo` or ` ` usually selects the session you want
